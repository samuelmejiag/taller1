defmodule PaqueteTuristico do
  ...
  @moduledoc """
  Módulo encubierto de la lógica de negocio para la cotización de paquetes turísticos.
...
@doc """
  Punto de entrada principal para la terminal interactiva.
  ...
  def main do
    ...
    destino = Util.leer("Ingrese el código del destino (1: Bogotá, 2: Medellín, 3: Cartagena, 4: San Andrés): ", :integer)
    ...
    resultado = calcular_paquete(destino, noches, cliente, mes, maleta, seguro)
    mostrar_resultado(resultado)
  end


  @doc """
  Coordina el proceso de validación encadenado de los parámetros de entrada.
...
@doc """
  Imprime en la pantalla el resumen legible del paquete cotizado o el mensaje de error.

  
  """

  def mostrar_resultado({:ok, {
    {tarifa_vuelo, valor_maleta, origen_maleta},
    {tarifa_noche, subtotal_hotel, descuento, recargo},
    {seguro, descuento_paquete, total}
  }}) do
    str_origen = case origen_maleta do
  ...
  def mostrar_resultado({:error, motivo}) do
    ...
    Util.imprimir_mensaje("Error al calcular el paquete: #{motivo}")
  end
